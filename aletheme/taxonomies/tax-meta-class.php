...
class Tax_Meta_Class {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_template_directory_uri() . '/aletheme/taxonomies';
  ...
  public function check_field_time() {
    ...
    if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
      ...
      // Enqueu JQuery UI, use proper version.
      $this->enqueue_jqueryui();
      wp_enqueue_script( 'at-timepicker', ALETHEME_THEME_URL . '/aletheme/taxonomies/js/jquery-ui-timepicker-addon/1.4.5/jquery-ui-timepicker-addon.min.js', array( 'tmc-jquery-ui' ),false,true );
    }    
  }
  ...
  public function show_field_repeater( $field, $meta ) {
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <span class="at-re-toggle"><img src="';
           if ($this->_Local_images){
             echo esc_attr($plugin_path).'/images/edit.png';
           }else{
        ...
        <img src="';
        if ($this->_Local_images){
          echo esc_attr($plugin_path).'/images/remove.png';
        }else{
          echo 'http://i.imgur.com/g8Duj.png';
        }
        echo '" alt="'.esc_html__('Remove','ale').'" title="'.esc_html__('Remove','ale').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '<img src="';
    if ($this->_Local_images){
      echo esc_attr($plugin_path).'/images/add.png';
    }else{
      echo 'http://i.imgur.com/w5Tuc.png';
    }
    echo '" alt="'.esc_html__('Add','ale').'" title="'.esc_html__('Add','ale').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  if (!isset($value['id']))
	...
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".esc_html__('Remove Image','ale')."' type='button'/>";
	else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".esc_html__('Upload Image','ale')."' type='button'/>";
	$this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__( 'Select a color','ale' ) . "'/>";
